      ...
      Carrito.findOne({ usuarioId }, (err, checkCarrito) => {
        if (!checkCarrito) {
          ...
          Carrito.findOne({ usuario: usuarioId }, (err, carritoId) => {
            ...
            Carrito.findOneAndUpdate(
              { _id: carritoId._id },
              { $push: { productos: data }, total: data.total },
              { new: true },
              (err, subirCarrito) => {
...
function eliminarProductoCarrito(req, res) {
  const productosId = req.params.idProductos;

  Carrito.findOneAndUpdate(
    { productos: { $elemMatch: { _id: productosId } } },
    { $pull: { productos: { _id: productosId } } },
    { new: true },
...
function eliminarCarrito(req, res) {
  Carrito.findOneAndDelete(
    ...
    (err, eliminarCarrito) => {
      if (err) return res.status(500).send({ mensaje: "Error en la peticion" });
      if (!eliminarCarrito)
        return res.status(500).send({ mensaje: "...Ups Algo salio mal" });
